...
function renderNoWallet(): string {
  return [
    ...
    "",
    "YOU need to run this step yourself — it requires an interactive terminal prompt for the mnemonic, which an agent can't provide. Open a terminal on the SAME machine where the skill was installed (your laptop, or the server running your agent — wherever the install one-liner just ran) and run:",
    "",
    "```",
    "poly login",
    "```",
    ...
    "",
    "After login, run this (or have your agent run it):",
    "",
    "```",
    "poly setup",
    "```",
    "",
    "and it will show you the next step. Every subsequent step can be agent-driven; login is the only one that needs you at a keyboard.",
  ].join("\n");
}
...
function renderNoGas(eoa: string, balances: { matic: string; usdcE: string; pUsd: string }): string {
  return [
    ...
    "",
    "This step is external — happens in your wallet/CEX, not on this machine. After the transfer confirms, run (or have your agent run):",
    "",
    "```",
    "poly setup",
    "```",
...
function renderNoUsdce(eoa: string, balances: { matic: string; usdcE: string; pUsd: string }): string {
  return [
    ...
    "",
    "After the transfer confirms (~30s on Polygon), run (or have your agent run):",
    "",
    "```",
    "poly setup",
    "```",
...
function renderNeedsWrap(
  ...
  return [
    ...
    "",
    "Run on this machine (or have your agent run it for you):",
    "",
    "```",
...
function renderNeedsAllowances(
  ...
  return [
    ...
    "",
    "Almost done — the trading allowances aren't set yet. Run this one-time approval (you or your agent — no TTY needed):",
    "",
    "```",
    "poly fund 0 --confirm",
    "```",
...
function renderReady(
  ...
  return [
    ...
    "Allowances:  set",
    "",
    "You're all set. From here on, just chat with your agent — it can run search, quote, bet, sell, close, positions, fund top-up, etc. directly without you typing in a terminal. Examples:",
    "",
    "```",
